"""
Multi-Scale BDH Training Script
================================

Complete training pipeline for multi-scale BDH model.
Trains on any text dataset using byte-level tokenization.

Features:
- Multi-scale state management (3 timescales)
- Automatic mixed precision (AMP) for faster training
- Gradient clipping for stability
- Learning rate scheduling with warmup
- Checkpoint saving and loading with state persistence
...
from multiscale_bdh import MultiScaleBDH, MultiScaleBDHConfig, count_parameters


@dataclass
class TrainingConfig:
    """Training hyperparameters for multi-scale BDH"""
    # Data
    ...
    hebbian_lr: float = 0.001  # Lower for multi-scale stability
    ...
    # State management
    use_states: bool = True  # Enable state persistence during training
    reset_states_interval: int = 0  # Reset states every N iterations (0 = never)

    def __post_init__(self):
        """Set default multi-scale parameters if not specified."""
        if self.decay_rates is None:
            self.decay_rates = [0.95, 0.99, 0.995]
...
def estimate_loss(
    ...
    for split, loader in [("train", train_loader)] + (
        ...
        for i, (x, y) in enumerate(loader):
            ...
            # Forward pass (without states for evaluation)
            logits, _ = model(x, states=None, return_states=False)
...
@torch.no_grad()
def measure_state_retention(
    model: nn.Module,
    config: TrainingConfig,
    device: str
) -> dict:
    """
    Measure state retention at different distances.

    This test verifies that multi-scale states retain information
    longer than single-scale states.
    ...
    Returns:
        Dictionary with retention metrics at different distances
    """
    model.eval()

    # Create a test pattern
    test_pattern = "The secret code is 12345. " * 20
    test_bytes = test_pattern.encode('utf-8')
    test_tensor = torch.tensor([list(test_bytes[:config.max_seq_len])], dtype=torch.long).to(device)

    # Initialize states
    _, states = model(test_tensor, states=None, return_states=True)

    # Measure state norms (proxy for information retention)
    retention = {}
    for i, (decay_rate, name) in enumerate(zip(config.decay_rates, ["fast", "medium", "slow"])):
        # Get first layer's state for this scale
        if states and len(states) > 0:
            state_norm = states[0][i].norm().item()
            retention[name] = state_norm
        else:
            retention[name] = 0.0
    ...
    if states and len(states) > 0:
        combined = sum(
            w * s.norm().item()
            for w, s in zip(config.scale_weights, states[0])
        )
        retention["combined"] = combined
    else:
        retention["combined"] = 0.0
...
def train_step(
    ...
    """
    Single training step with multi-scale state management.

    Args:
        model: The model
        x: Input tokens
        y: Target tokens
        states: Previous states or None
        optimizer: Optimizer
        config: Training configuration
    ...
    with torch.cuda.amp.autocast(dtype=config.dtype):
        # Use states if enabled
        if config.use_states:
            logits, states = model(x, states=states, return_states=True)
        else:
            logits, states = model(x, states=None, return_states=False)
...
def main():
    ...
    config = TrainingConfig(
        ...
        # State management
        use_states=True,
        reset_states_interval=0,  # Don't reset states
    )
    ...
    model_config = MultiScaleBDHConfig(
    ...
    model = MultiScaleBDH(model_config).to(config.device)
    ...
    # ========== TRAINING LOOP ==========
    print("\n" + "="*60)
    print("STARTING MULTI-SCALE BDH TRAINING")
    print("="*60 + "\n")
    ...
    for it in range(start_iter, config.max_iters):
        ...
        if config.reset_states_interval > 0 and it % config.reset_states_interval == 0:
            states = None

        # Training step
        loss, states = train_step(
            model, x, y, states, optimizer, config, scaler, it
        )
        ...
        if it % config.eval_interval == 0 and it > 0:
            ...
            # Measure state retention
            if config.use_states and config.device == "cuda":
                retention = measure_state_retention(model, config, config.device)
                print("State retention (norms):")
