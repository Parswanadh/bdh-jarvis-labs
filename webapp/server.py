...
class ModelService:
    ...
    def ensure_models_loaded(self) -> None:
        try:
            ...
            if self._is_cuda_runtime_error(e) and self._switch_to_cpu_and_reload(e):
                return
            raise
    ...
    def _load_bdh(self) -> None:
        ...
        model_runtime_cfg = checkpoint.get("model_config", {}) or {}

        def _cfg_get(name: str, default: Any) -> Any:
            for source in (runtime_cfg, model_runtime_cfg):
...
def parse_args() -> AppConfig:
    ...
    p.add_argument("--checkpoint", default="checkpoints/safe/laptop_safe_11L.pt")
    p.add_argument("--data", default="data/tinystories.txt")
    p.add_argument("--teacher-model", default="Qwen3.5-0.8B")
    p.add_argument("--distil-model", default="distilbert/distilgpt2")
    p.add_argument("--bdh-n-embd", type=int, default=256)
    p.add_argument("--bdh-n-layer", type=int, default=11)
    p.add_argument("--bdh-n-head", type=int, default=8)
    p.add_argument("--bdh-ffn-dim", type=int, default=1024)
