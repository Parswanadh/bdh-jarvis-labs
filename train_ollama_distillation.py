"""
Train Multi-Scale BDH with Ollama Gemma Distillation
=====================================================

Train Multi-Scale BDH using knowledge distillation from Ollama Gemma 3 270M.

Teacher: Ollama Gemma 3 270M (local)
...
class OllamaDataset(Dataset):
    """Dataset from Ollama teacher-generated data."""

    def __init__(self, data_file: str, max_seq_len: int = 512):
        """Load Ollama-generated dataset."""

        print(f"[LOAD] Loading Ollama teacher data from {data_file}")
...
def train_epoch(model, dataloader, optimizer, config, device, scaler=None):
    """Train for one epoch with mixed precision for speed."""

    model.train()
    total_loss = 0
    start_time = time.time()

    # Create GradScaler for mixed precision if needed
...
def train(config, train_config):
    ...
    try:
        ...
        print("[OK] Model compiled with torch.compile (FASTER!)")
    except:
        print("[INFO] torch.compile not available (using standard mode)")
    ...
    train_dataset = OllamaDataset(
        "data/ollama_generated/tinystories_data.jsonl",
    ...
    # Mixed precision setup for SPEED
    ...
    for iteration in range(train_config.max_iters):
        ...
        if (iteration + 1) % train_config.save_interval == 0:
            ...
            if (iteration + 1) % train_config.eval_interval == 0:
                ...
                print()
                print("="*60)
                print(f"[STATS] VALIDATION [iter {iteration + 1}]")
...
def main():
    ...
    print("="*60)
    print("BDH + OLLAMA DISTILLATION TRAINING")
    print("Teacher: Ollama Gemma 3 270M (local)")
    ...
    train_config.max_iters = 5  # 5 EPOCHS for better quality
    train_config.batch_size = 64  # DOUBLED from 32 (faster!)
    train_config.eval_interval = 1  # Evaluate every epoch
    train_config.save_interval = 1  # Save checkpoint every epoch
    train_config.checkpoint_dir = "checkpoints/multiscale_bdh_ollama"
    ...
    print(f"   Teacher: Ollama Gemma 3 270M")
    print(f"   Student: Multi-Scale BDH")
    print(f"   Training: 5 epochs (~1.5-2 hours total)")
