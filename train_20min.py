...
class TinyStoriesDataset(Dataset):
    def __init__(self, data_file, tokenizer, max_samples=800000, max_seq_len=512):
...
def main():
    print("="*70)
    print("20-MINUTE BDH TRAINING - 800K Stories")
    print("="*70)
    ...
    # Load dataset
    dataset = TinyStoriesDataset("data/tinystories.txt", tokenizer, max_samples=800000)
    ...
    total_loss = 0
    start = time.time()
    target_time = 20 * 60  # 20 minutes in seconds
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        # Check if we've hit 20 minutes
        elapsed = time.time() - start
        if elapsed >= target_time:
            print(f"\n[TIME] Reached 20 minutes! Stopping training.")
            break
