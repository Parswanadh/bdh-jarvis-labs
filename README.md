...
### Autonomous Self-Improvement (Web + Books Distillation)

Use curated JSONL data (from web snapshots, books, papers) and run one safe update cycle:
...
python run_autonomous_cycle.py ^
  ...
  --out-checkpoint checkpoints/autonomous/cycle_1.pt ^
  --epochs 1 --batch-size 8 --max-seq-len 256
```
...
```json
{
  ...
  "teacher_response": "Distilled target text",
  "source": "web",
...
The autonomous loop in `implementation/autonomous_self_improvement.py` includes:
- source reliability weighting (`web`, `books`, `papers`, `synthetic`)
- meta-cognitive confidence/uncertainty scoring from model logits
- safety-gated updates with rollback if validation degrades

For RunPod deployment steps, see `RUNPOD_AUTONOMOUS_WORKFLOW.md`.
